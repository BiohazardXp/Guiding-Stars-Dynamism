...
function Sidebar() {
  ...
  const handleLogout = () => {
    ...
    // Redirect to the appropriate login page based on the role that just logged out
  ...
  return (
    <>
      ...
      {isMobileMenuOpen && (
        <div onClick={() => setIsMobileMenuOpen(false)} className="lg:hidden fixed inset-0 bg-black bg-opacity-50 z-40" />
      )}

      <aside className={`fixed left-0 top-0 h-screen w-64 bg-gray-800 text-white shadow-lg flex flex-col z-40 transition-transform duration-300 ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        
        {/* Header - Dynamic Title */}
        <div className="p-6 border-b border-gray-700">
          <h1 className="text-2xl font-bold text-white">Guiding Stars</h1>
        ...
        <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
          ...
          {role === 'admin' && (
            <>
              <Link to="/mentors" className={navLinkClass('/mentors')} onClick={closeMobileMenu}>
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
                <span className="font-medium">Mentors</span>
              </Link>

              <Link to="/mentees" className={navLinkClass('/mentees')} onClick={closeMobileMenu}>
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" /></svg>
                <span className="font-medium">Mentees List</span>
              </Link>

              <Link to="/matches" className={navLinkClass('/matches')} onClick={closeMobileMenu}>
          ...
          {/* MENTEE ONLY LINKS */}
          {role === 'mentee' && (
            <Link to="/progress" className={navLinkClass('/progress')} onClick={closeMobileMenu}>
        ...
        <div className="p-4 border-t border-gray-700">
          <div className="flex items-center gap-3 px-4 py-3 bg-gray-700 rounded-lg mb-3">
            <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${role === 'admin' ? 'bg-blue-600' : 'bg-green-600'}`}>
